...
def get_input_dict(img_tensor,idx,dtype = torch.float32):
    ...
    return {'img':img_tensor, 'true_shape': np.array([[h, w]], dtype=np.int32), 'idx': idx, 'instance': str(idx), 'img_ori':img_tensor.to(dtype)}


def rotate_theta(c2ws_input, theta, phi, r, device): 
    # theta: The inclination of the image, the angle between the new y’ axis (located on the yoz plane) and the y axis
    # To let the camera move on a spherical surface with [0,0,depth_avg] as the center of the sphere, you can first let it move on the spherical surface with [0,0,0] as the center of the sphere to facilitate calculation of the rotation matrix, and then translate
    c2ws = copy.deepcopy(c2ws_input)
    c2ws[:,2, 3] = c2ws[:,2, 3] + r  # Translate the camera coordinate system by r along the world coordinate system -z direction
    ...
    # Compute the square of an antisymmetric matrix
    ...
    c2ws[:,2, 3]= c2ws[:,2, 3] - r # Finally, subtracting r is equivalent to rotating around z=|r|
...
def sphere2pose(c2ws_input, theta, phi, r, device):
    ...
    # First translate along the z-axis direction of the world coordinate system and then rotate
    if r is not None:
        c2ws[:,2,3] += r
...
def world_to_kth(poses, k):
    # Change the world coordinate system to be consistent with the camera coordinate system of the kth pose
    kth_pose = poses[k]
...
def txt_interpolation(input_list,n,mode = 'smooth'):
    ...
    if mode == 'smooth':
        ...
        raise KeyError(f"Invalid interpolation mode: {mode}")
